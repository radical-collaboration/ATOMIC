...
| size comes from | `queue_info/job_allocation` + `sysinfo`, overridable with `--declare` | the flags you pass (`--queue --nodes --cpus --walltime`) |
| pilots | one, started at join time, lives as long as the allocation | submitted on demand, up to `--max-pilots` |
| budget | `--node-hours`, else nodes × remaining walltime, else 1.0 | `--node-hours`, **required** |
...
3. **Detects capabilities.**  `sysinfo` is session-scoped, so the CLI
   registers a session, reads `metrics/{sid}` (`cpu.cores_logical`,
   `memory.total`, `gpus`) and unregisters it again.  In allocation mode
   it also reads the session-less `queue_info/job_allocation`
   (`n_nodes`, remaining `runtime`).  **`--declare` always wins** over
   what was detected, per key: `--declare cores=4` keeps the detected
   GPU count and memory.  Detection is best effort — if `sysinfo` is not
   there, declare what matters (at least `cores`).
4. **Joins** — `POST /broker/federation/join/default` with the resource
   record (name, endpoint, mode, site, kind, capabilities, budget,
   optional `scratch_base`, and in login mode the `pool` block).  The
   federation answers with the full record, including the pool
   (`fed-<name>`) it created for the resource.
...
| `--token TOK` | `$RADICAL_ORBIT_TOKEN`, then `$RADICAL_ORBIT_BROKER_TOKEN` | may be empty for a `--no-auth` broker — no `Authorization` header is then sent |
| `--cert PATH` | `$RADICAL_ORBIT_BROKER_CERT` | the broker's certificate, used to verify the connection |
...
| `--node-hours H` | the budget this join contributes (required in login mode) |
...
`ORBIT_SPEC` (a pip spec, a local path, or `none` to skip),
`ORBIT_BRANCH`, `ATOMIC_SPEC`, `ATOMIC_FORCE=1` to reinstall,
...
| `~/.radical/orbit/atomic/<name>/endpoint.pid` | pidfile written on every successful join, read by `atomic-leave`; a clean teardown removes it (so if the CLI itself is `SIGKILL`ed, `atomic-leave` can still stop the endpoint) |
...
**Leftover processes** — `atomic-leave` stops the endpoint recorded in
the pidfile *and* any surviving pilot child (a process running an ORBIT
endpoint whose name carries the pool prefix `fed-<name>_`; psij's `local`
executor cancels only the wrapper job).  `pgrep -af radical-orbit` should
