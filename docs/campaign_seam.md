...
The input side has no verb of its own: a stage's inputs travel **in** the
`submit` task body as `inputs_b64` (`{name: base64}`), because a
federation pool is a capability class whose member is only chosen when
the task is dispatched — there is no target directory before that.
Placement (`resource`, `member`, `member_id`, `class`) is read off
`task()`, never off `submit()`, and any of it may be absent.
...
| `ExecutorMixin._run_replica` (allocate → entry point → `_handle_replica_done`) | `StageRunner._run_stage` (submit-with-inputs → poll → collect → manifest) | same shape, different execution substrate |
